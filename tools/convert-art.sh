#!/bin/zsh
# PICT -> PNG for everything in original/Development Stuff (via netpbm), then build
# the runtime assets. Requires: brew install netpbm; tools/.venv with pillow.
set -e
cd "$(dirname "$0")/.."
mkdir -p build/art-png
find "original/Development Stuff" -type f ! -name "*.obj" ! -name "Icon_" ! -name "*Read Me*" -print0 |
while IFS= read -r -d '' f; do
	rel="${f#original/Development Stuff/}"
